...
#request uniform "screen" iResolution
uniform vec2 iResolution;

#request uniform "iTime" iTime
uniform float iTime;
...
if __name__ == '__main__':
   ...
   for id in sys.argv[1:]:
      ...
      while f == None:
         try:
            if attempt == 0:
               ...
               fname = "%s-%d" % (name, attempt)
            if not os.path.exists(home+"/.config/glava/"+fname):
               os.makedirs(home+"/.config/glava/"+fname)
            f = open (home+"/.config/glava/"+fname+"/1.frag", "x")
      ...
      f.write (shadermain)
      f.close ()
      print ("wrote shader to \"%s\"" % (home+"/.config/glava/"+fname+"/1.frag"), file=sys.stderr)
